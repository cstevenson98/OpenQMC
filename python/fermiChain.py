...
def fermi_tight_binding(n, mu0, J, delta):
    ...
    # H_hop = reduce(lambda a, b: a + b, [fermi_lower(n, i).dag() * fermi_lower(n, i + 1) for i in range(n - 1)],
    #                zeros_n(n))
    # H_hop += -J * H_hop.dag()
    #
    # # Hhop =  -∆ ∑_i  ci† ci+1 + h.c.
    # H_aniso = reduce(lambda a, b: a + b, [fermi_lower(n, i) * fermi_lower(n, i + 1) for i in range(n - 1)], zeros_n(n))
    # H_aniso += -delta * H_aniso.dag()
